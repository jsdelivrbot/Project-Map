...
document.addEventListener("DOMContentLoaded", function () {
    ...
    var yelpParams = "";
    xhttp.open("Post", yelpBaseurl, true);
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhttp.send(
        "grant_type=client_credentials&client_id=iaRX4joOJIg9Da3oMz_raw&client_secret=ZACyDM68ze0l7H0AWWvkfvVyezfWEFSPIOOCCfgywawXjvKudecJE56SwgKFFb02"
    );
    xhttp.onreadystatechange = function () {
        if (xhttp.readyState == 4 && xhttp.status == 200) {
            var response = JSON.parse(xhttp.responseText);
            console.log(response);
            populateYelpReviews("paris-las-vegas-hotel-and-casino-las-vegas", response.access_token);
            populateYelpReviews("planet-hollywood-las-vegas-resort-and-casino-las-vegas", response.access_token);
            populateYelpReviews("aria-resort-and-casino-las-vegas-5", response.access_token);
            populateYelpReviews("ellis-island-hotel-casino-and-brewery-las-vegas", response.access_token);
            populateYelpReviews("the-mirage-las-vegas-3", response.access_token);
            populateYelpReviews("The+Cheesecake+Factory", response.access_token);
            populateYelpReviews("bacchanal-buffet-las-vegas-7", response.access_token);
        }
    }
});

function populateYelpReviews(yelpId, accessToken) {
    var yelpBaseurl = `https://api.yelp.com/v3/businesses/${yelpId}/reviews`;
    var xhttp = new XMLHttpRequest();
